...
const AdminCarts = () => {
    ...
    const handleViewDetails = async (cart) => {
        ...
        const details = await fetchCartDetails(cart.id_usuario);
    ...
    if (loading) {
        return <div>Cargando carritos...</div>;
    }
                            ...
                            <tr key={cart.id_usuario}>
                                ...
                                <td>
                                    <div className="actions">
                                        ...
                                        <button
                                            onClick={() => handleDelete(cart.id_usuario)}
                ...
                >
                    ...
                    >
                        <h2 style={{
                            ...
                            Detalles del Carrito #{selectedCart.id_usuario}
                        </h2>

                        <div style={{ marginBottom: '1.5rem' }}>
                            <p style={{ margin: '0.5rem 0', color: '#666', fontSize: '0.9rem' }}>
                                <strong>Fecha de creación:</strong> {formatDate(selectedCart.fecha_creacion)}
                            </p>
                            <p style={{ margin: '0.5rem 0', color: '#666', fontSize: '0.9rem' }}>
                        ...
                        <div style={{
                            ...
                            {cartDetails.length === 0 ? (
                                <p style={{ color: '#999', margin: 0 }}>No hay productos en este carrito</p>
                            ) : (
                                <div>
                                    {cartDetails.map((item, index) => (
                                        ...
                                        >
                                            ...
                                            <div style={{ flex: 1 }}>
                                                <div style={{ fontWeight: '500', color: '#4e342e' }}>
                                                    {item.productos?.nombre || `Producto ID: ${item.id_producto}`}
                                                </div>
                                                <div style={{ fontSize: '0.85rem', color: '#666' }}>
                                                    Cantidad: {item.cantidad} × ${item.precio_unitario_actual?.toLocaleString('es-CL')}
                                                </div>
                                            </div>
                                    ...
                                    <div style={{
                                        ...
                                        <strong style={{ fontSize: '1.1rem', color: '#4e342e' }}>Total:</strong>
                                        <strong style={{ fontSize: '1.3rem', color: '#4e342e' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
                            ...
                            <button
                                onClick={() => {
                                    handleDelete(selectedCart.id_usuario);
                                    setShowModal(false);
                                }}
                                className="btn-danger"
                            >
